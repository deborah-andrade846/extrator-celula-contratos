...
def ler_texto_com_ocr(arquivo_pdf):
    ...
    with pdfplumber.open(arquivo_pdf) as pdf:
        ...
        for i, pagina in enumerate(pdf.pages):
            ...
            # Limpeza de memória
            del img_pil, img_cv, img_cinza, img_suave, img_binarizada
            barra_progresso.progress((i + 1) / total_paginas, text=f"OCR de Alta Precisão: Página {i+1} de {total_paginas}")
...
if tipo_selecionado == "fiscal":
    usar_ocr = st.checkbox("🔍 Forçar Leitura por OCR com Visão Computacional (Lento - Alta Precisão)")
